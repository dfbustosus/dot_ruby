...
# Simple RESTful API using only standard libraries
require 'webrick'
require 'json'
require 'logger'
...
# Handle individual item routes with separate mount points
# GET /api/items/:id - Get a specific item
server.mount_proc '/api/items/1' do |request, response|
  handle_item_request(request, response, 1)
end

server.mount_proc '/api/items/2' do |request, response|
  handle_item_request(request, response, 2)
end

server.mount_proc '/api/items/3' do |request, response|
  handle_item_request(request, response, 3)
end

server.mount_proc '/api/items/4' do |request, response|
  handle_item_request(request, response, 4)
end

server.mount_proc '/api/items/5' do |request, response|
  handle_item_request(request, response, 5)
end

# Helper function to handle item requests
def handle_item_request(request, response, id)
  item = find_item(id)
...
# Start server
puts "Starting simple RESTful API server on http://localhost:4567"
